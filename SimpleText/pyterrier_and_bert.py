...
def intersect_rankings(bert_ranked_docs, pyterrier_ranked_docs):
    intersection = []

    for (qid_bert, docs_bert), (qid_pyterrier, docs_pyterrier) in zip(bert_ranked_docs, pyterrier_ranked_docs):
        assert qid_bert == qid_pyterrier, "Query IDs do not match"
        doc_ids_bert = {doc_id: score for doc_id, score in docs_bert}
        doc_ids_pyterrier = {doc_id: score for doc_id, score in docs_pyterrier}

        common_doc_ids = set(doc_ids_bert.keys()).intersection(set(doc_ids_pyterrier.keys()))
        intersected_docs = [(doc_id, doc_ids_bert[doc_id], doc_ids_pyterrier[doc_id]) for doc_id in common_doc_ids]

        # Sort the intersected_docs by the sum of BERT and PyTerrier scores in descending order
        sorted_intersected_docs = sorted(intersected_docs, key=lambda x: x[1] + x[2], reverse=True)

        # Select the top 100 elements
        top_100_intersected_docs = sorted_intersected_docs[:100]

        intersection.append((qid_bert, top_100_intersected_docs))

    return intersection

def format_trec(intersected_rankings):
    trec_lines = []

    for query_id, docs in intersected_rankings:
        rank = 1
        for doc_id, bert_score, pyterrier_score in docs:
            combined_score = bert_score + pyterrier_score
            trec_line = f"{query_id} Q0 {doc_id} {rank} {combined_score} combined_model"
...
retrieval(index, query_list, "pyterrier_rankings_normalized_by_topic.txt")
...
write_bert_rankings(bert_ranked_docs, "bert_rankings_normalized_by_topic.txt")
